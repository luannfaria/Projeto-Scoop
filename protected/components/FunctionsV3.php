			...
			case "kr_search_location":
				...
				if (!empty($location_data)) {
					...
					switch ($location_type) {
						...
						case 2:
							$and_location = '';
							$state_id = isset($location_data['state_id']) ? (int)$location_data['state_id'] : 0;
							$city_id = isset($location_data['city_id']) ? (int)$location_data['city_id'] : 0;
							if ($city_id > 0) {
						...
						case 1;
							$and_location = '';
							$area_id = isset($location_data['area_id']) ? (int)$location_data['area_id'] : 0;
							$city_id = isset($location_data['city_id']) ? (int)$location_data['city_id'] : 0;
							if ($area_id > 0) {
						...
						case "3":
						case 3;
							$postal_code = isset($location_data['postal_code']) ? (int)$location_data['postal_code'] : 0;
							$query = " AND a.merchant_id IN (
					   	    select merchant_id
					   	    from
					   	    {{view_location_rate}}
		...
		if ($method == "driving" || $method == "transit") {
			...
			if (is_object($data)) {
				if ($data->status == "OK") {
					...
					if ($unit == "M") {
						$distance = (int) $value * 0.000621371192;
					} else $distance = (int) $value * 0.001;
...
?>
	...
	public static function getAdminPaymentList()
	{
		...
		if (getOptionA('merchant_pix_enabled') == "yes") {
			$payment_available[] = 'pix';
		}
		if (getOptionA('merchant_multicard_enabled') == "yes") {
			$payment_available[] = 'multicard';
		}
	...
	public static function getMerchantPaymentList($merchant_id = '')
	{
		...
		if ($merchant_type == 2) {
			...
			if (getOption($merchant_id, 'merchant_pix_enabled') == "yes") {
				$payment_available[] = 'pix';
			}
			if (getOption($merchant_id, 'merchant_pagarme_enabled') == "yes") {
				$payment_available[] = 'Pagarme';
			}
			if (getOption($merchant_id, 'merchant_multicard_enabled') == "yes") {
				$payment_available[] = 'multicard';
			}
...
<?php
	...
	public static function reCheckDelivery($merchant_id = '', $data = '')
	{
		...
		if ($merchant_info = FunctionsV3::getMerchantById($merchant_id)) {
			...
			$distance = FunctionsV3::getDistanceBetweenPlot(
				$lat,
				$lng,
				$merchant_info['latitude'],
				$merchant_info['lontitude'],
				$distance_type
			);
			//dump($distance);
	...
	public static function sendBankInstructions($merchant = '', $data = '')
	{
		...
		if ($enabled && !empty($to)) {
			...
			if (!empty($subject)) {
				$subject = self::smarty(
					"restaurant_name",
					isset($merchant['restaurant_name']) ? $merchant['restaurant_name'] : '',
					$subject
				);
			}
		...
		foreach ($pattern as $key => $val) {
			switch ($key) {
				...
				case "total_amount":
					$tpl = self::smarty(
						$key,
						isset($data[$val]) ? Price_Formatter::formatNumber($data[$val]) : '',
						$tpl
					);
					break;
				...
				case "date_booking":
					$tpl = self::smarty(
						$key,
						isset($data[$val]) ? date("M d,Y", strtotime($data[$val]))  : '',
						$tpl
					);
					break;
				default:
					$tpl = self::smarty(
						$key,
						isset($data[$val]) ? clearString($data[$val]) : '',
						$tpl
					);
					break;
			}
	...
	public static function getLocationDeliveryFeeWithMinimum(
		$merchant_id = '',
		$default_fee = '',
	...
	{
		$list = self::MembershipType();
		$terms_list = self::InvoiceTerms();
	...
	public static function getMerchantPaymentListNew($merchant_id = '')
	{
		...
		switch ($merchant_type) {
			...
			default:
				...
				if (getOption($merchant_id, 'merchant_pagarme_enabled') == 1) {
					$payment_available[] = 'pagarme';
				}
	...
	public static function getMapProvider()
	{
		...
		$map_distance_results = (int) getOptionA('map_distance_results');
	...
	public static function canCancelOrderNew(
		$request_cancel = '',
		$date_created = '',
		$order_status = '',
		$order_locked = '',
	...
	/*INIT FUNCTIONS FRANCHISE */
	public static function getFranchise($id = '')
	{
	...
	public static function getAreaFranchise($id = '')
	{
	...
	public static function getNewOrdersFranchise($city = '')
	{
		$db_ext = new DbExt;


		$and = "and city_id='.$city.' ";
	...
	public static function merchantListFranchise($as_list = true, $with_select = false)
	{
		...
		$stmt = "SELECT * FROM
			{{merchant}}
			WHERE status in ('active')
			AND city_id=" . $city['cidade_atendida_id'] . "
			ORDER BY restaurant_name ASC
			";
		if ($with_select) {
			$data[''] = t("All restaurant");
	...
	/* END FUNCTIONS FRANCHISE */

	/*UPDATE COMISSAO PEDIDO PAGARME */

	public static function updateComissionOrderPagarme($order_id, $pagarme_fee)
	{
		$db = new DbExt();
		$params_update = array(
	...
	/*FIM UPDATE COMISSAO PEDIDO PAGARME */
}/* end class*/
