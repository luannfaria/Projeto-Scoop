...
class Ajax extends AjaxAdmin
{
	...
	public function merchantCommissionDetails()
	{
		...
		if ($res = $DbExt->rst($stmt)) {
			foreach ($res as $val) {
				...
				if (!$mc_enabled) {
					$val_percent_commision = $val['percent_commision'];
	...
	public function merchantSignUp2()
	{
		...
		'contact_phone' => removeMascara(isset($this->data['contact_phone']) ? $this->data['contact_phone'] : ''),
			...
			'abn' => removeMascara(isset($this->data['abn']) ? $this->data['abn'] : ''),
			'cpf' => removeMascara(isset($this->data['cpf']) ? $this->data['cpf'] : ''),
	...
	public function AddIngredients()
	{
		...
		if (isset($this->data['ingredients_name_trans'])) {
			if (okToDecode()) {
				$params['ingredients_name_trans'] = json_encode(
					$this->data['ingredients_name_trans'],
		...
		if (isset($this->data['id']) && is_numeric($this->data['id'])) {
			...
			$res = $command->update(
				...
				array(':ingredients_id' => addslashes($this->data['id']))
			);
			if ($res) {
				$this->code = 1;
			...
			if (isset($this->data['default_account_paypal'])) {
				if ($this->data['default_account_paypal'] == 2) {
					Yii::app()->functions->updateOption(
						...
						isset($this->data['account']) ? $this->data['account'] : '',
						$mtid
					);
				}
			}
	...
	public function updateMessageMerchant()
	{
			...
			$DbExt = new DbExt;
			if ($DbExt->updateData("{{message_merchant}}", $params, 'id_message', $this->data['id_message'])) {
				$this->code = 1;
				$this->msg = t("Successful");
			} else $this->msg = t("Failed cannot update records");
			...
			case "fax_package":
				...
				if ($this->data['payment_opt'] == "pyp" || $this->data['payment_opt'] == "stp") {
					...
					if (is_array($merchantinfo) && count($merchantinfo) >= 1) {
						...
						$tpl = smarty(
							'verify-payment-link',
							websiteUrl() . "/merchant/faxbankdepositverification/?ref=$ref",
							$tpl
						);
						...
						if (sendEmail($merchant_email, $bank_info['sender'], $bank_info['subject'], $tpl)) {
							if ($this->insertData("{{fax_package_trans}}", $params_insert)) {
								...
								//$FunctionsK=new FunctionsK();
								$FunctionsK->faxSendNotification(
									(array)$merchantinfo[0],
	...
	public function shipppingRates()
	{
		...
		Yii::app()->functions->updateOption(
			"shipping_enabled",
			isset($this->data['shipping_enabled']) ? $this->data['shipping_enabled'] : '',
			$mtid
		);
	...
	public function merchantBankDeposit()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_bankdeposit_enabled",
			isset($this->data['merchant_bankdeposit_enabled']) ? $this->data['merchant_bankdeposit_enabled'] : '',
			$merchant_id
		);

		Yii::app()->functions->updateOption(
			"merchant_deposit_sender",
			isset($this->data['merchant_deposit_sender']) ? $this->data['merchant_deposit_sender'] : '',
			$merchant_id
		);

		Yii::app()->functions->updateOption(
			"merchant_deposit_subject",
			isset($this->data['merchant_deposit_subject']) ? $this->data['merchant_deposit_subject'] : '',
			$merchant_id
		);

		Yii::app()->functions->updateOption(
			"merchant_deposit_instructions",
	...
	{
		$order_id = isset($this->data['ref']) ? (int)$this->data['ref'] : '';
	...
	public function dishList()
	{
		...
		if (is_array($rows) && count($rows) >= 1) {
			foreach ($rows as $val) {
	    		...
	    		<a href=\"javascript:;\" class=\"delete_item\" data-id=\"$val[dish_id]\" >" . Yii::t("default", "Delete") . "</a>
	    		</div>";
	...
	public function addVoucherNew()
	{
		...
		$params = array(
			...
			'max_number_use' => isset($this->data['max_number_use']) ? (int)$this->data['max_number_use'] : 0,
			'selected_customer' => isset($this->data['selected_customer']) ? json_encode($this->data['selected_customer']) : '',
		);

		$days_list = FunctionsV3::dayList();
		$days_input = isset($this->data['days']) ? $this->data['days'] : array();
	...
	public function addMessageNew(){
			$merchant_id= $this->data['joining_merchant'];
		if (is_array($merchant_id) && count($merchant_id) >= 1) {
			foreach ($merchant_id as $merchant_id_val) {
				...
				if ($this->insertData('{{message_merchant}}', $params)) {
					$this->details = Yii::app()->db->getLastInsertID();
					$this->code = 1;
					$this->msg = Yii::t("default", "Successful");
				} else $this->msg = t("ERROR: Something went wrong");
    ...
    /* PAGARME*/
	public function merchantPagarmeSettings()
	{


		$merchant_id = Yii::app()->functions->getMerchantID();
		Yii::app()->functions->updateOption(
			"merchant_pagarme_enabled",
			isset($this->data['merchant_pagarme_enabled']) ? $this->data['merchant_pagarme_enabled'] : '',
			$merchant_id
		);
		...
		Yii::app()->functions->updateOption(
			"merchant_pagarme_id",
			isset($this->data['merchant_pagarme_id']) ? $this->data['merchant_pagarme_id'] : '',
			$merchant_id
		);
...
<?php
			...
			if ($DbExt->updateData('{{order}}', $params, 'order_id', $order_id)) {
				...
				if ($admin_info = Yii::app()->functions->getAdminInfo()) {
					...
					if (FunctionsV3::checkTableFields('order_history', $new_fields)) {
						$params_history['update_by_id'] = (int) $admin_info->admin_id;
	...
	public function minTableRates()
	{
		...
		Yii::app()->functions->updateOption(
			"min_tables_enabled",
			isset($this->data['min_tables_enabled']) ? $this->data['min_tables_enabled'] : '',
			$mtid
		);
	...
	public function InitPlaceOrder()
	{
		...
		if ($transaction_type == "delivery" && empty($accurate_address_lat) && !$is_by_location) {
			try {
		...
		if ($transaction_type == "delivery" && !$is_by_location) {
	...
	public function InvoiceList()
	{
		...
		$aColumns = array(
			...
			'invoice_total', 'date_from', 'status', 'pdf_filename', 'pdf_boleto'
		);
	...
	public function MerchantIpaySettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_ipay_enabled",
			isset($this->data['merchant_ipay_enabled']) ? $this->data['merchant_ipay_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_ipay_merchant_key",
			isset($this->data['merchant_ipay_merchant_key']) ? $this->data['merchant_ipay_merchant_key'] : '',
			$mtid
		);
	...
	public function MerchantPiPaySettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_pipay_enabled",
			isset($this->data['merchant_pipay_enabled']) ? $this->data['merchant_pipay_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_pipay_merchant_id",
			isset($this->data['merchant_pipay_merchant_id']) ? $this->data['merchant_pipay_merchant_id'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_pipay_device_id",
			isset($this->data['merchant_pipay_device_id']) ? $this->data['merchant_pipay_device_id'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_pipay_store_id",
			isset($this->data['merchant_pipay_store_id']) ? $this->data['merchant_pipay_store_id'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_pipay_mode",
			isset($this->data['merchant_pipay_mode']) ? $this->data['merchant_pipay_mode'] : '',
			$mtid
		);
	...
	public function MerchantHubtelPaymentSettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_hubtel_enabled",
			isset($this->data['merchant_hubtel_enabled']) ? $this->data['merchant_hubtel_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_hubtel_client_id",
			isset($this->data['merchant_hubtel_client_id']) ? $this->data['merchant_hubtel_client_id'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_hubtel_client_secret",
			isset($this->data['merchant_hubtel_client_secret']) ? $this->data['merchant_hubtel_client_secret'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_hubtel_accountno",
			isset($this->data['merchant_hubtel_accountno']) ? $this->data['merchant_hubtel_accountno'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_hubtel_channel",
			isset($this->data['merchant_hubtel_channel']) ? $this->data['merchant_hubtel_channel'] : '',
			$mtid
		);
	...
	public function SofortMerchantSettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_sofort_enabled",
			isset($this->data['merchant_sofort_enabled']) ? $this->data['merchant_sofort_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_sofort_config_key",
			isset($this->data['merchant_sofort_config_key']) ? $this->data['merchant_sofort_config_key'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_sofort_lang",
			isset($this->data['merchant_sofort_lang']) ? $this->data['merchant_sofort_lang'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_sofort_fee",
			isset($this->data['merchant_sofort_fee']) ? $this->data['merchant_sofort_fee'] : '',
			$mtid
		);
	...
	public function MerchantJampieSettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_jampie_enabled",
			isset($this->data['merchant_jampie_enabled']) ? $this->data['merchant_jampie_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_jampie_email",
			isset($this->data['merchant_jampie_email']) ? $this->data['merchant_jampie_email'] : '',
			$mtid
		);
	...
	public function MerchantPointsSettings()
	{
		...
		Yii::app()->functions->updateOption(
			"mt_disabled_pts",
			isset($this->data['mt_disabled_pts']) ? $this->data['mt_disabled_pts'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_pts_earn_points_status",
			isset($this->data['mt_pts_earn_points_status']) ? json_encode($this->data['mt_pts_earn_points_status']) : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_pts_earning_points",
			isset($this->data['mt_pts_earning_points']) ? $this->data['mt_pts_earning_points'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_pts_earning_points_value",
			isset($this->data['mt_pts_earning_points_value']) ? $this->data['mt_pts_earning_points_value'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_pts_disabled_redeem",
			isset($this->data['mt_pts_disabled_redeem']) ? $this->data['mt_pts_disabled_redeem'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_pts_redeeming_point",
			isset($this->data['mt_pts_redeeming_point']) ? $this->data['mt_pts_redeeming_point'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_pts_redeeming_point_value",
			isset($this->data['mt_pts_redeeming_point_value']) ? $this->data['mt_pts_redeeming_point_value'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_points_apply_order_amt",
			isset($this->data['mt_points_apply_order_amt']) ? $this->data['mt_points_apply_order_amt'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_points_minimum",
			isset($this->data['mt_points_minimum']) ? $this->data['mt_points_minimum'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_points_max",
			isset($this->data['mt_points_max']) ? $this->data['mt_points_max'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_points_based_earn",
			isset($this->data['mt_points_based_earn']) ? $this->data['mt_points_based_earn'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_pts_earn_above_amount",
			isset($this->data['mt_pts_earn_above_amount']) ? $this->data['mt_pts_earn_above_amount'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_pts_enabled_add_voucher",
			isset($this->data['mt_pts_enabled_add_voucher']) ? $this->data['mt_pts_enabled_add_voucher'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"mt_pts_enabled_offers_discount",
			isset($this->data['mt_pts_enabled_offers_discount']) ? $this->data['mt_pts_enabled_offers_discount'] : '',
			$mtid
		);
	...
	public function MerchantWingSettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_wing_enabled",
			isset($this->data['merchant_wing_enabled']) ? $this->data['merchant_wing_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wing_mode",
			isset($this->data['merchant_wing_mode']) ? $this->data['merchant_wing_mode'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wing_loginid",
			isset($this->data['merchant_wing_loginid']) ? $this->data['merchant_wing_loginid'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wing_password",
			isset($this->data['merchant_wing_password']) ? $this->data['merchant_wing_password'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wing_biller",
			isset($this->data['merchant_wing_biller']) ? $this->data['merchant_wing_biller'] : '',
			$mtid
		);
	...
	public function PaymillMerchantSettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_paymill_enabled",
			isset($this->data['merchant_paymill_enabled']) ? $this->data['merchant_paymill_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_paymill_mode",
			isset($this->data['merchant_paymill_mode']) ? $this->data['merchant_paymill_mode'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_paymill_test_private_key",
			isset($this->data['merchant_paymill_test_private_key']) ? $this->data['merchant_paymill_test_private_key'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_paymill_test_public_key",
			isset($this->data['merchant_paymill_test_public_key']) ? $this->data['merchant_paymill_test_public_key'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_paymill_live_private_key",
			isset($this->data['merchant_paymill_live_private_key']) ? $this->data['merchant_paymill_live_private_key'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_paymill_live_public_key",
			isset($this->data['merchant_paymill_live_public_key']) ? $this->data['merchant_paymill_live_public_key'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_paymill_card_fee1",
			isset($this->data['merchant_paymill_card_fee1']) ? $this->data['merchant_paymill_card_fee1'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_paymill_card_fee2",
			isset($this->data['merchant_paymill_card_fee2']) ? $this->data['merchant_paymill_card_fee2'] : '',
			$mtid
		);
	...
	public function MerchantIpayAfricaSettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_ipay_africa_enabled",
			isset($this->data['merchant_ipay_africa_enabled']) ? $this->data['merchant_ipay_africa_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_ipay_africa_mode",
			isset($this->data['merchant_ipay_africa_mode']) ? $this->data['merchant_ipay_africa_mode'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_ipay_africa_vendor_id",
			isset($this->data['merchant_ipay_africa_vendor_id']) ? $this->data['merchant_ipay_africa_vendor_id'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_ipay_africa_hashkey",
			isset($this->data['merchant_ipay_africa_hashkey']) ? $this->data['merchant_ipay_africa_hashkey'] : '',
			$mtid
		);
	...
	public function merchantDixiPaySettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_dixipay_mode",
			isset($this->data['merchant_dixipay_mode']) ? $this->data['merchant_dixipay_mode'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_dixipay_enabled",
			isset($this->data['merchant_dixipay_enabled']) ? $this->data['merchant_dixipay_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_dixipay_username",
			isset($this->data['merchant_dixipay_username']) ? $this->data['merchant_dixipay_username'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_dixipay_password",
			isset($this->data['merchant_dixipay_password']) ? $this->data['merchant_dixipay_password'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_dixipay_account_code",
			isset($this->data['merchant_dixipay_account_code']) ? $this->data['merchant_dixipay_account_code'] : '',
			$mtid
		);
	...
	public function WireCardSettingsMerchant()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_wirecard_enabled",
			isset($this->data['merchant_wirecard_enabled']) ? $this->data['merchant_wirecard_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wirecard_mode",
			isset($this->data['merchant_wirecard_mode']) ? $this->data['merchant_wirecard_mode'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wirecard_customer_id",
			isset($this->data['merchant_wirecard_customer_id']) ? $this->data['merchant_wirecard_customer_id'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wiredcard_shopid",
			isset($this->data['merchant_wiredcard_shopid']) ? $this->data['merchant_wiredcard_shopid'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wirecard_secret",
			isset($this->data['merchant_wirecard_secret']) ? $this->data['merchant_wirecard_secret'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wirecard_customer_id_live",
			isset($this->data['merchant_wirecard_customer_id_live']) ? $this->data['merchant_wirecard_customer_id_live'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wiredcard_shopid_live",
			isset($this->data['merchant_wiredcard_shopid_live']) ? $this->data['merchant_wiredcard_shopid_live'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wirecard_secret_live",
			isset($this->data['merchant_wirecard_secret_live']) ? $this->data['merchant_wirecard_secret_live'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wirecard_display_text",
			isset($this->data['merchant_wirecard_display_text']) ? $this->data['merchant_wirecard_display_text'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wirecard_lang",
			isset($this->data['merchant_wirecard_lang']) ? $this->data['merchant_wirecard_lang'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wirecard_fee_1",
			isset($this->data['merchant_wirecard_fee_1']) ? $this->data['merchant_wirecard_fee_1'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_wirecard_fee_2",
			isset($this->data['merchant_wirecard_fee_2']) ? $this->data['merchant_wirecard_fee_2'] : '',
			$mtid
		);
	...
	public function payulatamMerchantSettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_payulatam_enabled",
			isset($this->data['merchant_payulatam_enabled']) ? $this->data['merchant_payulatam_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_payulatam_mode",
			isset($this->data['merchant_payulatam_mode']) ? $this->data['merchant_payulatam_mode'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_payulatam_apikey",
			isset($this->data['merchant_payulatam_apikey']) ? $this->data['merchant_payulatam_apikey'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_payulatam_apilogin",
			isset($this->data['merchant_payulatam_apilogin']) ? $this->data['merchant_payulatam_apilogin'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_payulatam_mtid",
			isset($this->data['merchant_payulatam_mtid']) ? $this->data['merchant_payulatam_mtid'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_payulatam_apikey_live",
			isset($this->data['merchant_payulatam_apikey_live']) ? $this->data['merchant_payulatam_apikey_live'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_payulatam_apilogin_live",
			isset($this->data['merchant_payulatam_apilogin_live']) ? $this->data['merchant_payulatam_apilogin_live'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_payulatam_mtid_live",
			isset($this->data['merchant_payulatam_mtid_live']) ? $this->data['merchant_payulatam_mtid_live'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_payulatam_account_id",
			isset($this->data['merchant_payulatam_account_id']) ? $this->data['merchant_payulatam_account_id'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_payulatam_account_id_live",
			isset($this->data['merchant_payulatam_account_id_live']) ? $this->data['merchant_payulatam_account_id_live'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_payulatam_lang",
			isset($this->data['merchant_payulatam_lang']) ? $this->data['merchant_payulatam_lang'] : '',
			$mtid
		);
	...
	public function saveCategorySked()
	{
		...
		Yii::app()->functions->updateOption(
			...
			isset($this->data['enabled_category_sked']) ? $this->data['enabled_category_sked'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"enabled_category_sked_time",
			isset($this->data['enabled_category_sked_time']) ? $this->data['enabled_category_sked_time'] : '',
			$mtid
		);
	...
	public function MerchantPaystackSettings()
	{
		...
		Yii::app()->functions->updateOption(
			"merchant_paystack_enabled",
			isset($this->data['merchant_paystack_enabled']) ? $this->data['merchant_paystack_enabled'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_paystack_mode",
			isset($this->data['merchant_paystack_mode']) ? $this->data['merchant_paystack_mode'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_paystack_sandbox_secret_key",
			isset($this->data['merchant_paystack_sandbox_secret_key']) ? $this->data['merchant_paystack_sandbox_secret_key'] : '',
			$mtid
		);

		Yii::app()->functions->updateOption(
			"merchant_paystack_production_secret_key",
			isset($this->data['merchant_paystack_production_secret_key']) ? $this->data['merchant_paystack_production_secret_key'] : '',
			$mtid
		);
	...
	public function AllOrdersMerchant()
	{
		...
		if ($merchant_id > 0) {
			...
			$aColumns = array(
				'a.order_id', 'a.merchant_id', 'c.first_name',
				'json_details', 'trans_type', 'payment_type',
				'sub_total', 'taxable_total', 'total_w_tax', 'a.status', 'request_from', 'a.date_created'
			);
			$sWhere = '';
	...
	{
		$tag_id = isset($this->data['tag_id']) ? (int)$this->data['tag_id'] : 0;
		$params = array(
		...
		if (isset($this->data['tag_name_trans'])) {
			if (okToDecode()) {
				$params['tag_name_trans'] = json_encode(
					$this->data['tag_name_trans'],
					JSON_UNESCAPED_UNICODE
				);
		...
		if (isset($this->data['description_trans'])) {
			if (okToDecode()) {
				$params['description_trans'] = json_encode(
					$this->data['description_trans'],
					JSON_UNESCAPED_UNICODE
				);
	...
	public function OrderTimeManagmentList()
	{
		...
		if ($res = Yii::app()->db->createCommand($stmt)->queryAll()) {
			foreach ($res as $val) {
				...
				$option = "<div class=\"options\">
    	    		<a href=\"javascript:;\" class=\"add_time_management\" data-id=\"$val[group_id]\"  >" . Yii::t("default", "Edit") . "</a>
    	    		<a href=\"javascript:;\" class=\"delete_item\" data-id=\"$val[group_id]\" >" . Yii::t("default", "Delete") . "</a>
    	    		</div>";

				$feed_data['aaData'][] = array(
					$val['group_id'],
					t($val['transaction_type']) . $option,
					$days_inline,
					FunctionsV3::prettyTime($val['start_time']),
					FunctionsV3::prettyTime($val['end_time']),
	...
	/* NOVAS FUNÇOES VERSAO CUSTOMIZADA NOVEMBRO*/

	/* PIX */
	public function merchantPixSettings()
	{
		$merchant_id = Yii::app()->functions->getMerchantID();

		Yii::app()->functions->updateOption(
			"merchant_pix_enabled",
			isset($this->data['merchant_pix_enabled']) ? $this->data['merchant_pix_enabled'] : '',
			$merchant_id
		);

		Yii::app()->functions->updateOption(
			"merchant_pix_key",
			isset($this->data['merchant_pix_key']) ? $this->data['merchant_pix_key'] : '',
			$merchant_id
		);
...
/* FUNÇAO PARA REMOVER MASCARA DOS CAMPOS E SALVAR APENAS NUMEROS NO DB*/
