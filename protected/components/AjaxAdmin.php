	...
	class AjaxAdmin extends DbExt
	{
		...
		public function addMerchant()
		{
			...
			if (isset($params['is_featured'])) {
				$params['is_featured'] = (int)$params['is_featured'];
			}
			if (isset($params['is_ready'])) {
				$params['is_ready'] = (int)$params['is_ready'];
			}
			if (isset($params['is_commission'])) {
				$params['is_commission'] = (int)$params['is_commission'];
			}
			...
			$merchant_id = isset($this->data['id']) ? (int)$this->data['id'] : 0;
		...
		public function addCategory()
		{
			...
			if (isset($this->data['category_name_trans'])) {
				if (okToDecode()) {
					$params['category_name_trans'] = json_encode(
						$this->data['category_name_trans'],
			...
			if (isset($this->data['category_description_trans'])) {
				if (okToDecode()) {
					$params['category_description_trans'] = json_encode(
						$this->data['category_description_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				$res = $command->update(
					...
					$params,
					'cat_id=:cat_id',
					array(':cat_id' => addslashes($this->data['id']))
				);
				if ($res) {
					$this->code = 1;
		...
		public function addAddOnCategory()
		{
			...
			if (isset($this->data['subcategory_name_trans'])) {
				if (okToDecode()) {
					$params['subcategory_name_trans'] = json_encode(
						$this->data['subcategory_name_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				$res = $command->update(
					...
					$params,
					'subcat_id=:subcat_id',
					array(':subcat_id' => addslashes($this->data['id']))
				);
				if ($res) {
					$this->code = 1;
					$this->msg = Yii::t("default", 'SubCategory updated.');

					$subcat_id = (int) $this->data['id'];
				} else $this->msg = Yii::t("default", "ERROR: cannot update");
			} else {
		...
		public function addOnItemNew()
		{
			...
			if (isset($this->data['sub_item_name_trans'])) {
				if (okToDecode()) {
					$params['sub_item_name_trans'] = json_encode(
						$this->data['sub_item_name_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['item_description_trans'])) {
				if (okToDecode()) {
					$params['item_description_trans'] = json_encode(
						$this->data['item_description_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				$res = $command->update(
					...
					array(':sub_item_id' => addslashes($this->data['id']))
				);
				if ($res) {
					$this->code = 1;
		...
		public function AddSize()
		{
			...
			if (isset($this->data['size_name_trans'])) {
				if (okToDecode()) {
					$params['size_name_trans'] = json_encode(
						$this->data['size_name_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				$res = $command->update(
					...
					$params,
					'size_id=:size_id',
					array(':size_id' => addslashes($this->data['id']))
				);
				if ($res) {
					$this->code = 1;
					$this->msg = Yii::t("default", 'Size updated.');

					$size_id = (int) $this->data['id'];
				} else $this->msg = Yii::t("default", "ERROR: cannot update");
			} else {
		...
		public function AddCookingRef()
		{
			...
			if (isset($this->data['cooking_name_trans'])) {
				if (okToDecode()) {
					$params['cooking_name_trans'] = json_encode(
						$this->data['cooking_name_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				$res = $command->update(
					...
					$params,
					'cook_id=:cook_id',
					array(':cook_id' => addslashes($this->data['id']))
				);
				if ($res) {
					$this->code = 1;
		...
		public function FoodItemAdd()
		{
			...
			if (isset($this->data['item_name_trans'])) {
				if (okToDecode()) {
					$params['item_name_trans'] = json_encode(
						$this->data['item_name_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				$res = $command->update(
					...
					$params,
					'item_id=:item_id',
					array(':item_id' => addslashes($this->data['id']))
				);
				if ($res) {
					$this->code = 1;
		...
		public function merchantSettings()
		{
			...
			Yii::app()->functions->updateOption(
				"merchant_opt_contact_delivery",
				isset($this->data['merchant_opt_contact_delivery']) ? $this->data['merchant_opt_contact_delivery'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"website_merchant_time_picker_interval",
				isset($this->data['website_merchant_time_picker_interval']) ? $this->data['website_merchant_time_picker_interval'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"tracking_estimation_delivery1",
				isset($this->data['tracking_estimation_delivery1']) ? (int)$this->data['tracking_estimation_delivery1'] : 0,
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"tracking_estimation_delivery2",
				isset($this->data['tracking_estimation_delivery2']) ? (int)$this->data['tracking_estimation_delivery2'] : 0,
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"tracking_estimation_pickup1",
				isset($this->data['tracking_estimation_pickup1']) ? (int)$this->data['tracking_estimation_pickup1'] : 0,
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"tracking_estimation_pickup2",
				isset($this->data['tracking_estimation_pickup2']) ? (int)$this->data['tracking_estimation_pickup2'] : 0,
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"tracking_estimation_dinein1",
				isset($this->data['tracking_estimation_dinein1']) ? (int)$this->data['tracking_estimation_dinein1'] : 0,
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"tracking_estimation_dinein2",
				isset($this->data['tracking_estimation_dinein2']) ? (int)$this->data['tracking_estimation_dinein2'] : 0,
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_menu_lazyload",
				isset($this->data['merchant_menu_lazyload']) ? (int)$this->data['merchant_menu_lazyload'] : 0,
				$merchant_id
			);
			...
			Yii::app()->functions->updateOption(
				...
				isset($this->data['merchant_service_fee_applytax']) ? (int)$this->data['merchant_service_fee_applytax'] : 0,
				$merchant_id
			);
...
?>
		...
		public function addToCart()
		{
			...
			if ($website_disabled_cart_validation != 2) :
				...
				$h_item_id = isset($this->data['item_id']) ? (int)$this->data['item_id'] : '';
			...
			if (isset($this->data['item_id'])) {
				...
				if (is_numeric($this->data['row'])) {
					...
					if (FunctionsV3::inventoryEnabled($this->data['merchant_id'])) {
						...
						$current_item_id = isset($item['item_id']) ? (int)$item['item_id'] : '';
						$current_item_price = isset($item['price']) ? $item['price'] : '';
						$current_item_size = isset($item['with_size']) ? (int)$item['with_size'] : 0;
						$current_item_size = $current_item_size == 2 ? 1 : 0;
						$inv_qty = isset($item['qty']) ? (int)$item['qty'] : 0;
		...
		public function setDeliveryOptions()
		{
			...
			if (Yii::app()->functions->isClientLogin()) {
				try {
					...
					CheckoutWrapper::verifyCanPlaceOrder((int) $client_id);
				} catch (Exception $e) {
					$this->msg = $e->getMessage();
					return;
				}
			...
			$merchantid = isset($this->data['merchant_id']) ? (int)$this->data['merchant_id'] : 0;
			...
			$_SESSION['kr_delivery_options']['opt_contact_delivery'] = isset($this->data['opt_contact_delivery']) ? (int)$this->data['opt_contact_delivery'] : 0;
		...
		public function clientLogin()
		{
			...
			if (Yii::app()->functions->clientAutoLogin(
				$this->data['username'],
				$this->data['password'],
				$this->data['password_md5']
			)) {
		...
		public function placeOrder()
		{
			...
			if (
				$enabled_map_selection_delivery != 1 && empty($accurate_address_lng) &&
			...
			if (
				$transaction_type == "delivery" && empty($accurate_address_lng)
			...
			if (isset($this->data['client_order_sms_code'])) {
				if (!empty($this->data['client_order_sms_code'])) {
					if (!FunctionsK::validateOrderSMSCode(
			...
			if (isset($this->data['is_guest_checkout'])) {
				...
				if ($guest_exist) {
					...
					if ($this->insertData("{{client}}", $params)) {
						...
						FunctionsV3::deleteCustomerGuest((int) $new_client_id, $params['email_address']);
						Yii::app()->functions->clientAutoLogin($this->data['email_address'], $this->data['password']);
			...
			if (is_array($order_item) && count($order_item) >= 1) {
				...
				switch ($this->data['payment_opt']) {
					case "pyp":
						if (FunctionsV3::isMerchantPaymentToUseAdmin($this->data['merchant_id'])) {
							...
							$card_fee = Yii::app()->functions->getOption(
								'merchant_paypal_fee',
								$this->data['merchant_id']
							);
						}
				...
				if (Yii::app()->functions->code == 1) {
					...
					$merchantinfo = Yii::app()->functions->getMerchant($this->data['merchant_id']);
					...
					if (is_array($raw) && count($raw) >= 1) {
						...
						/*FIXED ORDER STATUS*/
						if (
							$this->data['payment_opt'] == "cod" || $this->data['payment_opt'] == "pyr" ||
						...
						if (isset($_SESSION['voucher_code'])) {
							...
							if ($_SESSION['voucher_code']['voucher_type'] == "fixed amount") {
								$params['voucher_value'] = (float)$_SESSION['voucher_code']['amount']  * $exchange_rate;
							} else {
								$subtotal = isset($raw['total']['t']) ? $raw['total']['subtotal'] : '';
								$total = $subtotal;
								$percert = $_SESSION['voucher_code']['amount'];
								$params['voucher_value'] = (float)$_SESSION['voucher_code']['amount']  * $exchange_rate;
							}
						}
						...
						if ($this->insertData("{{order}}", $params)) {
							...
							if ($transaction_type == "delivery") {
								$params_address = array(
									...
									'area_name' => isset($this->data['area_name']) ? $p->purify($this->data['area_name']) : '',
									'date_created' => FunctionsV3::dateNow(),
									'ip_address' => $_SERVER['REMOTE_ADDR'],
									'contact_phone' => $p->purify($this->data['contact_phone']),
									...
									'opt_contact_delivery' => isset($this->data['opt_contact_delivery']) ? (int)$this->data['opt_contact_delivery'] : 0
								);
							...
							$params_address['order_id'] = (int)$order_id;
							$params_address['client_id'] = (int)$client_id;
							...
							if ($service_resp) {
								...
								$params_address['service_fee_applytax'] = (int)$service_resp['service_fee_applytax'];
							}
							...
							if ($this->data['saved_address'] == 2) {
								if (!FunctionsV3::isSearchByLocation()) {
									...
									$params_loation_address = array(
										...
										'country_id' => isset($this->data['country_id']) ? (int) $this->data['country_id'] : 0,
										'state_id' => isset($this->data['state_id']) ? (int) $this->data['state_id'] : '',
										'city_id' => isset($this->data['city_id']) ? (int)$this->data['city_id'] : '',
										'area_id' => isset($this->data['area_id']) ? (int) $this->data['area_id'] : '',
										'as_default' => 1,
										'date_created' => FunctionsV3::dateNow(),
										'ip_address' => $_SERVER['REMOTE_ADDR'],
										'latitude' => $accurate_address_lat,
							...
							foreach ($raw['item'] as $val) {
								...
								if (FunctionsV3::checkTableFields('order_details', $new_fields)) {
									$params_order_details['size_id'] = isset($val['size_id']) ? (int) $val['size_id'] : 0;
									$params_order_details['cat_id'] = isset($val['category_id']) ? (int) $val['category_id'] : 0;
								}
										...
										if (is_array($val['sub_item']) && count($val['sub_item']) >= 1) {
											foreach ($val['sub_item'] as $sub_item_data) {
												Yii::app()->db->createCommand()->insert("{{order_details_addon}}", array(
													...
													'subcat_id' => (int)$sub_item_data['subcat_id'],
													'sub_item_id' => (int)$sub_item_data['sub_item_id'],
			...
			if (Yii::app()->functions->isClientLogin()) {
				...
				if ($DbExt->insertData("{{review}}", $params)) {
					...
					if (isset($this->data['initial_review_rating'])) {
						Yii::app()->functions->updateRatings(
							$this->data['merchant-id'],
							$this->data['initial_review_rating'],
							$client_id
						);
					}
		...
		public function savePaypalSettings()
		{
			...
			Yii::app()->functions->updateOption(
				"enabled_paypal",
				isset($this->data['enabled_paypal']) ? $this->data['enabled_paypal'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"paypal_mode",
				isset($this->data['paypal_mode']) ? $this->data['paypal_mode'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"sanbox_paypal_user",
				isset($this->data['sanbox_paypal_user']) ? $this->data['sanbox_paypal_user'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"sanbox_paypal_pass",
				isset($this->data['sanbox_paypal_pass']) ? $this->data['sanbox_paypal_pass'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"sanbox_paypal_signature",
				isset($this->data['sanbox_paypal_signature']) ? $this->data['sanbox_paypal_signature'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"live_paypal_user",
				isset($this->data['live_paypal_user']) ? $this->data['live_paypal_user'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"live_paypal_pass",
				isset($this->data['live_paypal_pass']) ? $this->data['live_paypal_pass'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"live_paypal_signature",
				isset($this->data['live_paypal_signature']) ? $this->data['live_paypal_signature'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_paypal_fee",
				isset($this->data['merchant_paypal_fee']) ? $this->data['merchant_paypal_fee'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"mt_paypal_mobile_enabled",
				isset($this->data['mt_paypal_mobile_enabled']) ? $this->data['mt_paypal_mobile_enabled'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"mt_paypal_mobile_mode",
				isset($this->data['mt_paypal_mobile_mode']) ? $this->data['mt_paypal_mobile_mode'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"mt_paypal_mobile_clientid",
				isset($this->data['mt_paypal_mobile_clientid']) ? $this->data['mt_paypal_mobile_clientid'] : '',
				$merchant_id
			);
	           ...
	           die();*/
				...
				if ($res = $paypal->expressCheckout()) {
					...
					$command->update(
						...
						$params,
						'token=:token',
						array(':token' => addslashes($this->data['token']))
					);
					...
					$command->update(
						...
						$params,
						'order_id=:order_id',
						array(':order_id' => addslashes($this->data['order_id']))
					);
									...
									foreach ($val['sub_item'] as $subitem_key => $subitem_val) {
										foreach ($subitem_val as $subitem_key2 => $subitem_val2) {
											...
											if (is_array($subitem) && count($subitem) >= 1) {
												$sub_item_id = isset($subitem[0]) ? (int) $subitem[0] : 0;
...
// INICIO ALTERAÇÕES LISTAR STATUS PAINEL MERCHANT
			$status_list = Yii::app()->functions->orderStatusListView();
			if ($this->data['currentController'] == "admin") {
				$status_list = Yii::app()->functions->orderStatusList();
			}

			// FIM ALTERAÇÕES LISTAR STATUS PAINEL MERCHANT
			?>
			...
			<script type="text/javascript">
				$('#status_selected').on('change', function() {
					var status_selecionado = this.value;
					alert(status_selecionado);
					//  alert(status_selecionado); //or alert($(this).val());
					if ((status_selecionado == 'cancelled') || (status_selecionado == 'declined') || (status_selecionado == 'decline')) {
						$("#remarks").attr("data-validation", "required");

						//	$(".action-wrap").hide();
					} else {
						$("#remarks").removeAttr("data-validation");
		...
		public function updateOrder()
		{
			...
			if (isset($this->data['order_id'])) {
				...
				if ($resp = Yii::app()->functions->verifyOrderIdByOwner($order_id, $merchant_id)) {
					...
					$can_edit = (int) Yii::app()->functions->getOptionAdmin('merchant_days_can_edit_status');
					...
					if ($can_edit > 0 || !empty($edit_times)) {
						...
						if (is_array($date_interval) && count($date_interval) >= 1) {
							...
							if (!empty($edit_times)) {
								...
								$hour = isset($times[0]) ? (int)$times[0] : 0;
								$minute = isset($times[1]) ? (int)$times[1] : 0;
								if ($hour > 0) {
						...
						if ($merchant_info = Yii::app()->functions->getMerchantInfo()) {
							...
							if (FunctionsV3::checkTableFields('order_history', $new_fields)) {
								if ($kr_merchant_user_type == "merchant_user") {
									...
									$params_history['update_by_id'] = (int) $merchant_info->merchant_user_id;
									$params_history['update_by_name'] = "$merchant_info->first_name $merchant_info->last_name";
								} else {
									$params_history['update_by_type'] = 'merchant';
									$params_history['update_by_id'] = (int) $merchant_info->merchant_id;
		...
		public function chartToArea()
		{
			$merchant_id = Yii::app()->functions->getMerchantID();

			$data = array();
			$db_ext = new DbExt();
			$date_now = date('Y-m-d 23:59:00');
			$start_date = date('Y-m-d 00:00:00', strtotime($date_now . "-30 days"));
			$stmt = "SELECT a.zipcode as area,
	    	       count(a.zipcode) as total,
	    	       b.date_created
	    	       FROM
	    	       {{order_delivery_address}} a
	    	       left join {{order}} b
	    	       ON
	    	       a.order_id=b.order_id
	    	       WHERE
	    	       b.date_created BETWEEN " . FunctionsV3::q($start_date) . " AND " . FunctionsV3::q($date_now) . "
	    	       AND b.merchant_id = " . FunctionsV3::q($merchant_id) . "
	    	       AND b.status NOT IN ('" . initialStatus() . "')
				   AND b.trans_type ='delivery'
	    	       GROUP BY a.zipcode
	    	       ORDER BY a.zipcode DESC
	    	";
			//dump($stmt);
			if ($res = $db_ext->rst($stmt)) {
				foreach ($res as $val) {

					$data[Yii::t("default", $val['area'])] =  $val['total'];
				}
			}
			echo Yii::app()->functions->formatAsChart($data);
			die();
		...
		public function adminSettings()
		{
			...
			Yii::app()->functions->updateOptionAdmin(
				"admin_menu_lazyload",
				isset($this->data['admin_menu_lazyload']) ? (int)$this->data['admin_menu_lazyload'] : ''
			);
			...
			Yii::app()->functions->updateOptionAdmin(
				"admin_service_fee_applytax",
				isset($this->data['admin_service_fee_applytax']) ? (int)$this->data['admin_service_fee_applytax'] : 0
			);

			Yii::app()->functions->updateOptionAdmin(
				"captcha_driver_signup",
				isset($this->data['captcha_driver_signup']) ? (int)$this->data['captcha_driver_signup'] : 0
			);

			Yii::app()->functions->updateOptionAdmin(
				"mobile2_hide_empty_category",
				isset($this->data['mobile2_hide_empty_category']) ? (int)$this->data['mobile2_hide_empty_category'] : 0
			);

			Yii::app()->functions->updateOptionAdmin(
				"merchant_time_can_edit_status",
				isset($this->data['merchant_time_can_edit_status']) ? trim($this->data['merchant_time_can_edit_status']) : ''
			);

			Yii::app()->functions->updateOptionAdmin(
				"captcha_contact_form",
				isset($this->data['captcha_contact_form']) ? (int)$this->data['captcha_contact_form'] : 0
			);
		...
		public function merchantSignUp()
		{
			...
			if (!Yii::app()->functions->validateUsername($this->data['username'])) {
				...
				if ($respck = Yii::app()->functions->validateMerchantUserFromMerchantUser(
					$params['username'],
				...
				if ($res = $this->rst($stmt)) {
					if ($res[0]['status'] == "active") {
						...
						if ($res[0]['activation_key'] == $this->data['activation_code']) {
							...
							if ($package_info = FunctionsV3::getMerchantPaymentMembership(
								$res[0]['merchant_id'],
								$res[0]['package_id']
							)) {
...
<?php
		...
		{
			$p = new CHtmlPurifier();
			$cuisine_id = isset($this->data['id']) ? (int) $this->data['id'] : 0;
			...
			if ($Validator->validate()) {
				...
				if (isset($this->data['cuisine_name_trans'])) {
					if (okToDecode()) {
						$params['cuisine_name_trans'] = json_encode(
							$this->data['cuisine_name_trans'],
							JSON_UNESCAPED_UNICODE
						);
		...
		public function contacUsSubmit()
		{
			...
			if ($validator->validate()) {
				...
				if (!empty($tpl)) {
					...
					$tpl = FunctionsV3::smarty(
						'phone',
						isset($this->data['phone']) ? $this->data['phone'] : '',
						$tpl
					);
				...
				case "email":
					...
					if ($Validator->validate()) {
						...
						if ($res = yii::app()->functions->isClientExist($email)) {
							...
							$up = Yii::app()->db->createCommand()->update(
								"{{client}}",
								$params,
								'client_id=:client_id',
								array(
							...
							$this->code = 1;
							$this->msg = t("We sent your forgot password link, Please follow that link. Thank You.");
							$this->details = array(
				...
				case "sms":
					...
					if ($Validator->validate()) {
						try {
							...
							if ($sms['msg'] == "process") {
								$this->code = 1;
								$this->msg = t("We have sent verification code in your phone number");
								$this->details = array(
									'verification_type' => $forgot_pass_action,
		...
		public function addCustomPage()
		{
			...
			if (isset($this->data['page_name_trans'])) {
				if (okToDecode()) {
					$params['page_name_trans'] = json_encode(
						$this->data['page_name_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['content_trans'])) {
				if (okToDecode()) {
					$params['content_trans'] = json_encode(
						$this->data['content_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['seo_title_trans'])) {
				if (okToDecode()) {
					$params['seo_title_trans'] = json_encode(
						$this->data['seo_title_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['meta_description_trans'])) {
				if (okToDecode()) {
					$params['meta_description_trans'] = json_encode(
						$this->data['meta_description_trans'],
						JSON_UNESCAPED_UNICODE
					);
			...
			if (isset($this->data['meta_keywords_trans'])) {
				if (okToDecode()) {
					$params['meta_keywords_trans'] = json_encode(
						$this->data['meta_keywords_trans'],
						JSON_UNESCAPED_UNICODE
					);
		...
		public function smsSettings()
		{
			...
			Yii::app()->functions->updateOptionAdmin(
				"locasms_user",
				isset($this->data['locasms_user']) ? $this->data['locasms_user'] : ''
			);

			Yii::app()->functions->updateOptionAdmin(
				"locasms_pass",
				isset($this->data['locasms_pass']) ? $this->data['locasms_pass'] : ''
			);
		...
		public function merchantMercadoSettings()
		{
			...
			Yii::app()->functions->updateOption(
				"merchant_mercado_enabled",
				isset($this->data['merchant_mercado_enabled']) ? $this->data['merchant_mercado_enabled'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_mercado_mode",
				isset($this->data['merchant_mercado_mode']) ? $this->data['merchant_mercado_mode'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_mercado_id",
				isset($this->data['merchant_mercado_id']) ? $this->data['merchant_mercado_id'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_mercado_key",
				isset($this->data['merchant_mercado_key']) ? $this->data['merchant_mercado_key'] : '',
				$merchant_id
			);
...
$last_login=$val['last_login']=="0000-00-00 00:00:00"?"":date('M d,Y G:i:s',strtotime($val['last_login']));  */
		...
		public function addMerchantUser()
		{
		    ...
		    */
			...
			if (empty($this->data['id'])) {
				...
				if ($err_msg = $FunctionsK->validateMerchantUserAccount(
					$this->data['username'],
		...
		public function merchantPaylineSettings()
		{
			...
			Yii::app()->functions->updateOption(
				"merchant_payline_enabled",
				isset($this->data['merchant_payline_enabled']) ? $this->data['merchant_payline_enabled'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_payline_mode",
				isset($this->data['merchant_payline_mode']) ? $this->data['merchant_payline_mode'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_payline_api",
				isset($this->data['merchant_payline_api']) ? $this->data['merchant_payline_api'] : '',
				$merchant_id
			);
		...
		public function bookATableMerchant()
		{
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				$res = $command->update(
					...
					array(':booking_id' => addslashes($this->data['id']))
				);
				if ($res) {
					$this->code = 1;
		...
		public function customerAdd()
		{
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				$res = $command->update(
					...
					$params,
					'client_id=:client_id',
					array(':client_id' => addslashes($this->data['id']))
				);
				if ($res) {
					$this->code = 1;
		...
		public function merchantPayseraSettings()
		{
			...
			Yii::app()->functions->updateOption(
				"merchant_paysera_enabled",
				isset($this->data['merchant_paysera_enabled']) ? $this->data['merchant_paysera_enabled'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_paysera_mode",
				isset($this->data['merchant_paysera_mode']) ? $this->data['merchant_paysera_mode'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_paysera_project_id",
				isset($this->data['merchant_paysera_project_id']) ? $this->data['merchant_paysera_project_id'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_paysera_password",
				isset($this->data['merchant_paysera_password']) ? $this->data['merchant_paysera_password'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_paysera_country",
				isset($this->data['merchant_paysera_country']) ? $this->data['merchant_paysera_country'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_paysera_lang",
				isset($this->data['merchant_paysera_lang']) ? $this->data['merchant_paysera_lang'] : '',
				$merchant_id
			);
		...
		public function addOffers()
		{
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				if (!Yii::app()->functions->getMerchantOffers(
					$merchant_id,
					$this->data['valid_from'] . " 00:00:00",
					$this->data['valid_to'] . " 00:00:00",
					...
					$res = $command->update(
						...
						$params,
						'offers_id=:offers_id',
						array(':offers_id' => addslashes($this->data['id']))
					);
					if ($res) {
						$this->code = 1;
		...
		public function saveMerchantBarclaySettings()
		{
			...
			Yii::app()->functions->updateOption(
				"merchant_enabled_barclay",
				isset($this->data['merchant_enabled_barclay']) ? $this->data['merchant_enabled_barclay'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_mode_barclay",
				isset($this->data['merchant_mode_barclay']) ? $this->data['merchant_mode_barclay'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_sandbox_barclay_pspid",
				isset($this->data['merchant_sandbox_barclay_pspid']) ? $this->data['merchant_sandbox_barclay_pspid'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_sandbox_barclay_password",
				isset($this->data['merchant_sandbox_barclay_password']) ? $this->data['merchant_sandbox_barclay_password'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_live_barclay_pspid",
				isset($this->data['merchant_live_barclay_pspid']) ? $this->data['merchant_live_barclay_pspid'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_live_barclay_password",
				isset($this->data['merchant_live_barclay_password']) ? $this->data['merchant_live_barclay_password'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_currency",
				isset($this->data['merchant_bcy_currency']) ? $this->data['merchant_bcy_currency'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_language",
				isset($this->data['merchant_bcy_language']) ? $this->data['merchant_bcy_language'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_font",
				isset($this->data['merchant_bcy_font']) ? $this->data['merchant_bcy_font'] : '',
				$merchant_id
			);
			...
			Yii::app()->functions->updateOption(
				"merchant_bcy_bgcolor",
				isset($this->data['merchant_bcy_bgcolor']) ? $this->data['merchant_bcy_bgcolor'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_buttontextcolor",
				isset($this->data['merchant_bcy_buttontextcolor']) ? $this->data['merchant_bcy_buttontextcolor'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_table_bgcolor",
				isset($this->data['merchant_bcy_table_bgcolor']) ? $this->data['merchant_bcy_table_bgcolor'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_table_textcolor",
				isset($this->data['merchant_bcy_table_textcolor']) ? $this->data['merchant_bcy_table_textcolor'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_textcolor",
				isset($this->data['merchant_bcy_textcolor']) ? $this->data['merchant_bcy_textcolor'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_title",
				isset($this->data['merchant_bcy_title']) ? $this->data['merchant_bcy_title'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_buttoncolor",
				isset($this->data['merchant_bcy_buttoncolor']) ? $this->data['merchant_bcy_buttoncolor'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_listype",
				isset($this->data['merchant_bcy_listype']) ? $this->data['merchant_bcy_listype'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_bcy_logo",
				isset($this->data['merchant_bcy_logo']) ? $this->data['merchant_bcy_logo'] : '',
				$merchant_id
			);
		...
		public function saveMerchantEpaybgSettings()
		{
			...
			Yii::app()->functions->updateOption(
				"merchant_enabled_epaybg",
				isset($this->data['merchant_enabled_epaybg']) ? $this->data['merchant_enabled_epaybg'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_mode_epaybg",
				isset($this->data['merchant_mode_epaybg']) ? $this->data['merchant_mode_epaybg'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_sandbox_epaybg_min",
				isset($this->data['merchant_sandbox_epaybg_min']) ? $this->data['merchant_sandbox_epaybg_min'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_sandbox_epaybg_secret",
				isset($this->data['merchant_sandbox_epaybg_secret']) ? $this->data['merchant_sandbox_epaybg_secret'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_live_epaybg_min",
				isset($this->data['merchant_live_epaybg_min']) ? $this->data['merchant_live_epaybg_min'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_live_epaybg_secret",
				isset($this->data['merchant_live_epaybg_secret']) ? $this->data['merchant_live_epaybg_secret'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_sandbox_epaybg_request",
				isset($this->data['merchant_sandbox_epaybg_request']) ? $this->data['merchant_sandbox_epaybg_request'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_live_epaybg_request",
				isset($this->data['merchant_live_epaybg_request']) ? $this->data['merchant_live_epaybg_request'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_sandbox_epaybg_lang",
				isset($this->data['merchant_sandbox_epaybg_lang']) ? $this->data['merchant_sandbox_epaybg_lang'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_live_epaybg_lang",
				isset($this->data['merchant_live_epaybg_lang']) ? $this->data['merchant_live_epaybg_lang'] : '',
				$merchant_id
			);
		...
		public function merchantMulticardSettings()
		{
			$merchant_id = Yii::app()->functions->getMerchantID();

			Yii::app()->functions->updateOption(
				"merchant_multicard_enabled",
				isset($this->data['merchant_multicard_enabled']) ? $this->data['merchant_multicard_enabled'] : '',
				$merchant_id
			);

			Yii::app()->functions->updateOption(
				"merchant_cod_estabelecimento",
				isset($this->data['merchant_cod_estabelecimento']) ? $this->data['merchant_cod_estabelecimento'] : '',
				$merchant_id
			);
			...
			public function franchiseAreaList()
			{

				$admin_id = Yii::app()->functions->getAdminId();
				$admin_info = Yii::app()->functions->getAdminUserInfo($admin_id);
				if ($admin_info['type_user'] == 'franchise') {
					$city = Yii::app()->functions->getFranchiseInfo($admin_info['franchise_id']);
					//	$sWhere = 'where city_id='.$city['cidade_atendida_id'].'';
				}
				$slug = $this->data['slug'];
				$stmt = "SELECT * FROM
					{{location_area}}
					where city_id=" . $city['cidade_atendida_id'] . "

					";
				if ($res = $this->rst($stmt)) {
					foreach ($res as $val) {
						$action = "<div class=\"options\">
							<a href=\"$slug/id/$val[area_id]\" >" . Yii::t("default", "Edit") . "</a>
							<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[area_id]\" >" . Yii::t("default", "Delete") . "</a>
							</div>";
			...
			{
				$id_city = $this->data['cidadeatendida'][0];
				$name_city = Yii::app()->functions->getcidade($id_city);
				$params['razao_social'] = $this->data['razaosocial'];
				$params['cnpj'] = 	Yii::app()->functions->deixarNumero( $this->data['cnpj']);
			$params['telefone'] = Yii::app()->functions->deixarNumero($this->data['telefone']);
				$params['endereco'] = $this->data['endereco'];
				$params['cep'] = $this->data['cep'];
				$params['cidade_atendida_id'] = $this->data['cidadeatendida'][0];
				$params['taxa'] = $this->data['taxa'];

				$params['cidade'] = $name_city['name'];
				$params['estado'] = $this->data['estado'];
				$params['nome_franqueado'] = $this->data['nome'];
				$params['cpf_franqueado'] = $this->data['cpf'];
				$params['rg_franqueado'] = $this->data['rg'];
				$params['telefone_responsavel'] = $this->data['telefone_responsavel'];
				$params['email_responsavel'] = $this->data['email_responsavel'];
				$params['endereco_responsavel'] = $this->data['endereco_responsavel'];
				$params['cidade_responsavel'] = $this->data['cidade_responsavel'];
				$params['status'] = $this->data['status'];
				$params = FunctionsV3::purifyData($params);

				$id_franchise = isset($this->data['id']) ? (int)$this->data['id'] : 0;

				if ($id_franchise <= 0) {
					if ($this->insertData("{{franchise}}", $params)) {
					...
					$res = $this->updateData('{{franchise}}', $params, 'id', $this->data['id']);
					if ($res) {
						$this->code = 1;
						$this->msg = Yii::t("default", 'Franchise update.');
					}
				}
			}

			public function addAreaFranchise()
			{
				...
				$area_id = isset($this->data['area_id']) ? (int)$this->data['area_id'] : 0;

				if ($area_id <= 0) {
					if ($this->insertData("{{location_area}}", $params)) {
						$id = Yii::app()->db->getLastInsertID();
						$this->details = $id;
					...
					$res = $this->updateData('{{location_area}}', $params, 'area_id', $this->data['area_id']);
					if ($res) {
						$this->code = 1;
						$this->msg = Yii::t("default", 'Franchise update.');
					}
			...
			public function merchantListFranchise()
			{
				...
				$stmt = "SELECT * FROM
					{{merchant}}
					WHERE
					city_id = " . $city['cidade_atendida_id'] . "
					ORDER BY merchant_id DESC
					";
				if ($res = $this->rst($stmt)) {
					foreach ($res as $val) {
						...
						$feed_data['aaData'][] = array(
							...
							//			$val['activation_key'],
							//			membershipType($val['is_commission']),
							"<div class=\"uk-badge $class\">" . strtoupper(Yii::t("default", $val['status'])) . "</div>" . $link_login
						);
					}
					$this->otableOutput($feed_data);
				}
			...
			public function rptIncomingOrdersFranchise()
			{
				$admin_id = Yii::app()->functions->getAdminId();
				$admin_info = Yii::app()->functions->getAdminUserInfo($admin_id);
				...
				$stmt = "SELECT
					...
					WHERE a.status NOT IN ('" . initialStatus() . "')
						and a.city_id ='" .	$city['cidade_atendida_id'] . "'
					AND date_created LIKE '" . date("Y-m-d") . "%'
			...
			public function rptMerchantPaymentTodayFranchise()
			{
				$admin_id = Yii::app()->functions->getAdminId();
				$admin_info = Yii::app()->functions->getAdminUserInfo($admin_id);
				if ($admin_info['type_user'] == 'franchise') {
				...
				$and2 = "and 	city_id = " . $city['cidade_atendida_id'] . "";
				$and = '';
				$datenow = date('Y-m-d');
				$and = "WHERE date_created like '$datenow%' ";
