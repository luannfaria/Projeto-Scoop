...
class AjaxController extends CController
{
	...
	public function actionCityList()
	{
		$post = $_POST;
		$status = true;
		$state_id = isset($post['state_id']) ? (int)$post['state_id'] : 0;
	...
	public function actionAreaList()
	{
		...
		$id  = isset($post['city_id']) ? (int)$post['city_id'] : 0;
	...
	public function actionStateList()
	{
		$data = array();
		$status = false;
		$country_id = (int) FunctionsV3::getLocationDefaultCountry();
		$stmt = "
	...
	public function actionloadMenu()
	{
		...
		if ($this->data['merchant_id'] > 0) {
			...
			if (is_array($menu) && count($menu) >= 1) {
				...
				$menu_lazyload = getOptionA('admin_menu_lazyload');
				$lazy_use_mobile = false;
	...
	public function actionrequestCancelBooking()
	{
		...
		$booking_id = isset($this->data['id']) ? (int)$this->data['id'] : '';
	...
	{
		$name = isset($_POST['q']) ? $_POST['q'] : '';
		$merchant_id = isset($_POST['auto_merchant_id']) ? (int)$_POST['auto_merchant_id'] : '';
		$data = array();
		$status = false;
		...
		$and = '';
		$hide_food = getOption((int) $merchant_id, 'food_option_not_available');
		if ($hide_food == 1) {
	...
	{
		$merchant_id = isset($this->data['merchant_id']) ? (int)$this->data['merchant_id'] : 0;
		$today = strtolower(date("l"));
		$time_now = date("H:i");
		if (isset($this->data['date_selected'])) {
