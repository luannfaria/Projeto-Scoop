...
";
		...
		$stmt = "
		CREATE TABLE IF NOT EXISTS " . $prefix . "franchise (
			`id` int(14) NOT NULL AUTO_INCREMENT,
			...
			`taxa` int(11)  NULL,
			`nome_franqueado` varchar(100) NOT NULL,
			`cpf_franqueado` varchar(20)  NOT NULL,
			`rg_franqueado` varchar(50)  NOT NULL,
			`endereco_responsavel` varchar(150) NOT NULL,
			`cidade_responsavel` varchar(100)  NULL,
			`cep_responsavel` varchar(50) DEFAULT  NULL,
			`email_responsavel` varchar(80) DEFAULT  NULL,
			`telefone_responsavel` varchar(50) DEFAULT NULL,
			`cidade_atendida_id` varchar(11)   NOT NULL,
			`status` varchar(20) NOT NULL,
			PRIMARY KEY (`id`)) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;
			";

		echo "Creating Table franchise..<br/>";
		if (!Yii::app()->functions->isTableExist("franchise")) {
			if ($DbExt->qry($stmt)) {
				echo "(Done)<br/>";
			} else echo "(Failed)<br/>";
		} else echo "Table franchise already exist.<br/>";

		$new_field = array(
			'city_franchise_id' => "int(11) NULL DEFAULT '0'",
			'type_user' => "varchar(30) NOT NULL DEFAULT NULL",
		...
		$new_field = array(
			'city_id' => "int(11) NULL",
			'pagarme_fee' => "float(14,4) DEFAULT NULL",
		);
		$this->alterTable('order', $new_field);
		...
		$new_field = array(
			'total_pagarme_fee' => "float(14,4) DEFAULT NULL",
		);
		$this->alterTable('invoice', $new_field);
