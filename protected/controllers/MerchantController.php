...
class MerchantController extends CController
{
	...
	public function actionOBDinitOLD()
	{
		...
		if ($res = Yii::app()->functions->getSMSPackagesById($package_id)) {
			...
			if (!empty($merchant_email)) {
				...
				if (!empty($tpl)) {
					$tpl = Yii::app()->functions->smarty(
						'amount',
						Yii::app()->functions->adminCurrencySymbol() . Yii::app()->functions->standardPrettyFormat($amount_to_pay),
						$tpl
					);
	...
	public function actionPixSettings()
	{
		$this->crumbsTitle = Yii::t("default", "PIX settings");

		$this->render('pix-settings');
	}

	public function actionMulticardSettings()
	{
		$this->crumbsTitle = Yii::t("default", "Multicard settings");

		$this->render('multicard-settings');
	}

	public function actionQrCode()
	{
		$this->crumbsTitle = Yii::t("default", "Qr Code ");

		$this->render('qrcode');
	}
	public function actionHome()
	{
		$this->crumbsTitle = Yii::t("default", "Home");
		$this->render('home');
	}
	public function actionpagarmesettings()
	{

		$this->crumbsTitle = t("Pagarme");
		if (Yii::app()->functions->hasMerchantAccess("pagarme")) {
			$this->render('pagarme-settings', array(
				'mtid' => Yii::app()->functions->getMerchantID()
			));
		} else $this->render('error', array(
			'message' => t("Sorry but your not allowed to access this page")
