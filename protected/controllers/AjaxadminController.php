...
class AjaxadminController extends CController
{
	...
	public function actionMigration_items()
	{
		...
		$total_item = isset($this->data['total']) ? (int)$this->data['total'] : 0;
		$total_item_paginate = ceil((int)$total_item / (int) $page_limit);
	...
	{
		$this->data = $_POST;
		$page_limit = ItemClass::paginateMigrate();
		$data = array();
		if (isset($this->data['page'])) {
			$page = (int)$this->data['page'];
		} else  $page = 0;
		...
		if ($resp = Yii::app()->db->createCommand($stmt)->queryAll()) {
			foreach ($resp as $val) {
				...
				if ($fields = FunctionsV3::getLanguageList(false)) {
					...
					foreach ($fields as $lang_code) {
						$params = array(
							'sub_item_id' => (int)$val['sub_item_id'],
							'language' => $lang_code,
		...
		if ($resp = Yii::app()->db->createCommand($stmt)->queryAll()) {
			foreach ($resp as $val) {
				...
				if ($fields = FunctionsV3::getLanguageList(false)) {
					...
					foreach ($fields as $lang_code) {
						$params = array(
							'ingredients_id' => (int)$val['ingredients_id'],
							'language' => $lang_code,
	...
	public function actionMigration_itemsubitem()
	{
		...
		if ($res = Yii::app()->db->createCommand($stmt)->queryAll()) {
			foreach ($res as $val) {
				$addon_item = json_decode($val['addon_item'], true);
				ItemClass::insertItemRelationshipSubcategory((int)$val['merchant_id'], (int)$val['item_id'], (array)$addon_item);
			}
