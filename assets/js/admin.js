		...
		success: function (data) {
			...
			if (data.code == 1) {
				...
				switch (action) {
					...
					case "login":
						uk_msg(data.msg);
						if (typeof captcha_site_key === "undefined" || captcha_site_key == null || captcha_site_key == "") {} else {
							recaptchav3();
						}
						break;
...
jQuery(document).ready(function () {
	...
	$(document).on("click", ".closeAlert", function () {
		...
		updateMessageMerchant(id_message);
	});
	...
	if ($("#currentController").val() == "merchant") {
		new_order_notify = setInterval(function () {
			get_new_order()
...
function notificationDesktop() {
	...
	if (!("Notification" in window)) {
		...
		var options = {
			body: 'Você tem novo(s) pedido(s)!',
			icon: 'https://scoopdelivery.com.br/assets/images/default-image-merchant.png',
			status: "info"
		};

		notify = new Notification('Scoop!',
...
function load_map() {
	if ($("#merchant_latitude").val() == "" && $("#merchant_longtitude").val() == "") {
		...
		locations = [
			[$("#restaurant_name").val(), $("#merchant_latitude").val(), $("#merchant_longtitude").val(), 16]
		];
...
function load_total_sales_by_area() {
	$.jqplot.config.enablePlugins = true;
	var ajaxDataRenderer = function (url, plot, options) {
		var ret = null;
		$.ajax({
	...
	var jsonurl = ajax_url + "/?action=chartToArea&tbl=chart&currentController=" + $("#currentController").val();

	jsonurl += addValidationRequest();

	var plot5 = $.jqplot('total_sales_by_area', jsonurl, {
...
/*MESSAGE MERCHANT*/
function updateMessageMerchant(id_message) {
	var params = "action=updateMessageMerchant&id_message=" + id_message;
	params += addValidationRequest();
...
jQuery(document).ready(function () {
	...
	$(document).on("click", ".print_thermal", function () {
		...
		if (isChrome) {
			printing_window.onload = function () {
...
$.validate({
	...
	onSuccess: function () {
		...
		busy(true);
		$("#frm_redirect_payment button").css({
			'pointer-events': 'none'
		});
		return true;
	}
...
jQuery(document).ready(function () {
	...
	if (current_panel == "admin") {
		if (!empty(has_session)) {
			if (has_session == 1) {
				admin_cancel_order_handle = setInterval(function () {
					getNewCancelOrderAdmin()
...
mapbox_init_map = function (lat, lng) {
	if (!empty(lat) && !empty(lng)) {
		...
		if (empty(mapbox_handle)) {
			...
			mapbox_marker = L.marker([lat, lng], {
				draggable: true
			}).addTo(mapbox_handle);
		} else {
...
var processAjax = function (action, data, single_call, silent, method, loader_object) {
	...
	process_ajax_request[timenow] = $.ajax({
		...
		beforeSend: function (xhr) {
			if ((typeof silent !== "undefined") && (silent !== null)) {} else {
